const classes = ["統資一甲","統資一乙","統資二甲","統資二乙","統資三甲","統資三乙","統資四甲","統資四乙","碩研一","碩研二"];
...
function renderCards(){
  const filtered=allVoices().filter(v=>(state.className==="全部"||v.className===state.className)&&(state.topic==="全部"||v.topic===state.topic));
  ...
  filtered.slice(0,state.limit).forEach(v=>{
    ...
    article.dataset.id=v.id;card.querySelector(".class-tag").textContent=v.className;card.querySelector(".topic-tag").textContent=v.topic;
  ...
  $("#emptyState").hidden=filtered.length>0;$("#loadMore").hidden=state.limit>=filtered.length;
}
function renderHabitat(){
  $("#classHabitat").innerHTML=classes.map((c,i)=>`<button class="class-cell" data-class="${c}"><em>${String(i+1).padStart(2,"0")}</em><strong>${c}</strong><span>${allVoices().filter(v=>v.className===c).length}${i18n[state.lang].voicesUnit}</span><i aria-hidden="true"></i></button>`).join("");
...
function updateCount(){const n=326+userVoices.length;$("#heroCount").textContent=n;$("#voiceCount").textContent=n}
