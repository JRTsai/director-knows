...
const denseHeartRows=[
  {y:7,xs:[29,39,61,71]},
  {y:15,xs:[21,31,41,59,69,79]},
  {y:24,xs:[15,25,35,45,55,65,75,85]},
  {y:33,xs:[12,22,32,42,50,58,68,78,88]},
  {y:42,xs:[12,22,32,42,50,58,68,78,88]},
  {y:51,xs:[16,27,38,49,60,71,82,88]},
  {y:60,xs:[22,33,44,50,56,67,78]},
  {y:69,xs:[29,40,50,60,71]},
  {y:78,xs:[38,50,62]},
  {y:87,xs:[44,56]},
  {y:95,xs:[50]}
];
const denseHeartSlots=denseHeartRows.flatMap(row=>row.xs.map(x=>[x,row.y]));
function buildHeartTokens(ranked,total){
  const primary=ranked.map(([word,count])=>({word,count,showCount:true}));
  const weighted=ranked.flatMap(([word,count])=>Array.from({length:Math.max(1,count)},()=>({word,count,showCount:false})));
  const extras=[];
  while(primary.length+extras.length<total)extras.push({...weighted[Math.floor(Math.random()*weighted.length)]});
  return shuffleWords([...primary,...extras]).slice(0,total);
}
function renderWordCloud(){
  const ranked=extractKeywords(filteredVoices()),cloud=$("#wordCloud"),empty=$("#wordCloudEmpty");cloud.innerHTML="";cloud.classList.remove("sis-cloud");cloud.classList.add("heart-cloud","dense-heart-cloud");empty.hidden=ranked.length>0;
  if(!ranked.length)return;
  const max=Math.max(...ranked.map(([,count])=>count)),min=Math.min(...ranked.map(([,count])=>count)),slots=shuffleWords(denseHeartSlots),tokens=buildHeartTokens(ranked,slots.length);
  tokens.forEach(({word,count,showCount},i)=>{
    const item=document.createElement("span"),ratio=max===min?.45:(count-min)/(max-min),slot=slots[i],angles=word.length<=2?[-90,-18,-8,0,0,0,8,18,90]:[-12,-6,0,0,6,12],angle=angles[Math.floor(Math.random()*angles.length)];
    item.className=`cloud-word${showCount?"":" cloud-repeat"}`;item.style.setProperty("--weight",(.68+ratio*1.32).toFixed(2));item.style.setProperty("--cloud-color",`var(--cloud-${Math.floor(Math.random()*5)})`);item.style.setProperty("--x",`${slot[0]}%`);item.style.setProperty("--y",`${slot[1]}%`);item.style.setProperty("--rotate",`${angle}deg`);item.textContent=word;
    if(showCount){const n=document.createElement("sup");n.textContent=count;item.append(n);item.title=`${word}：${count}`;item.setAttribute("aria-label",`${word}，出現 ${count} 次`)}else{item.setAttribute("aria-hidden","true")}
    cloud.append(item)
  })
}
